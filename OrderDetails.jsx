...
export default function OrderDetails() {
  ...
  const timeline = useMemo(() => {
    ...
    return [...commentEvents, ...activityEvents].sort(
  ...
  async function handleAttachFiles(event) {
    ...
    try {
      ...
      toast({
        ...
        description: `${uploaded.length} file(s) are ready to send with the message.`,
      });
    } catch (uploadError) {
      toast({
        title: "Upload failed",
  ...
  async function handleDownload(file) {
    try {
      const fileName =
        ...
        "download";
      const fileUrl =
        file.fileUrl || `/uploads/${file.storedFileName || file.fileName}`;
      await downloadAttachment(fileUrl, fileName);
    } catch (err) {
      toast({
        title: "Download failed",
        description: err.message || "Unable to download this file.",
        variant: "destructive",
      });
    }
  }
  ...
  async function submitComment(event) {
    ...
    try {
      setCommenting(true);
      await addOrderCommentMutation.mutateAsync({
        orderId: id,
        text: draft,
      ...
      toast({
        ...
        description: "The thread has been updated in real time.",
      });
      setDraft("");
      setQueuedFiles([]);
    } catch (mutationError) {
  ...
  const currentIdx = ORDER_STATUS_OPTIONS.findIndex(
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      <button
        onClick={() => navigate("/client")}
        className="flex items-center gap-1.5 text-sm text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-100"
      >
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <Badge
          className={cn("text-sm px-3 py-1", ORDER_STATUS_STYLES[order.status])}
        >
          {order.status}
        </Badge>
      </div>
                    ...
                    <li key={status} className="ml-6">
                      ...
                      {idx === currentIdx && (
                        <p className="text-xs text-slate-400 mt-0.5">
                          Current stage as of {order.date}
                        </p>
                      )}
                    </li>
                  );
                      ...
                      <Badge
                        className={cn(
                          ...
                          ORDER_STATUS_STYLES[
                            invoice.status === "PAID"
                              ? "Completed"
                              : invoice.status === "PENDING_DEPOSIT"
                              ? "Review"
                              : "Pending"
                          ]
                        )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
          <Card className="lg:col-span-3 bg-white dark:bg-slate-900">
            ...
            <CardContent className="space-y-4">
              <form
                ...
                className="space-y-3 rounded-2xl border border-slate-200 dark:border-slate-800 p-4 bg-slate-50 dark:bg-slate-800/40"
              >
                <Textarea
                  rows={4}
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <button
                    type="button"
                    onClick={() => insertMention("@Atlas Team")}
                    className="inline-flex items-center gap-1 rounded-full border border-slate-300 dark:border-slate-700 px-3 py-1 text-xs text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700"
                  >
                  ...
                  {queuedFiles.length > 0 && (
                    <span className="text-xs text-slate-500">
                      {queuedFiles.length} attachment(s) queued
                    </span>
                  )}
                </div>
                      ...
                      <div className="flex items-center justify-between gap-3">
                        <div>
                          <p className="text-sm font-semibold text-slate-900 dark:text-slate-100">
                            {entry.type === "comment"
                              ? `${entry.user?.workspace?.name ?? "Team"} commented`
                              : entry.summary}
                          </p>
                          <p className="text-xs text-slate-400">
          ...
          <Card className="lg:col-span-2 bg-white dark:bg-slate-900 h-fit">
            ...
            <CardContent className="space-y-3 text-sm">
              ...
              <DetailRow
                label="Attachments"
                value={order.attachments?.length ?? 0}
              />
              <div className="rounded-xl bg-slate-50 dark:bg-slate-800 p-4 text-slate-600 dark:text-slate-300 text-xs">
                Messages and audit events are merged chronologically, so the
                thread doubles as the activity log.
              </div>
            </CardContent>
          </Card>
        </div>
